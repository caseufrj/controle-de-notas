...
from bd import conectar  # <<-- ajuste se seu arquivo com 'conectar()' tiver outro nome
...
def usuario_registrar(email: str, senha: str, nome: Optional[str] = None, role: str = "user") -> int:
    ...
    if not senha or len(senha) < 10:
        raise ValueError("Senha deve ter ao menos 10 caracteres (prefira passphrase).")
...
def usuario_login(email: str, senha: str,
    ...
    ok = _verificar_senha(row["senha_hash"], senha)
    if not ok:
        _registrar_auditoria("login_failure", row["id"], email_norm, ip, user_agent)
        conn.close()
        raise ValueError("Credenciais inválidas.")

    token = _sessao_criar(row["id"], duracao_horas=8, user_agent=user_agent, ip=ip)
    ...
    return {
        "token": token,
        "usuario": {"id": row["id"], "email": row["email"], "nome": row["nome"], "role": row["role"]}
    }
...
def usuario_por_token(token: str) -> Optional[Dict[str, Any]]:
    ...
    cur.execute("""
        SELECT u.id, u.email, u.nome, u.role
         ...
         WHERE s.id = ? AND s.expira_em >= datetime('now','localtime')
    """, (token,))
    row = cur.fetchone()
    conn.close()
...
def usuario_logout(token: str, ip: Optional[str] = None, user_agent: Optional[str] = None) -> None:
    ...
    r = cur.fetchone()
    uid = r["usuario_id"] if r else None
