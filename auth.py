# ======================================================================
# AUTH.PY — VERSÃO 2026 FINAL
# Suporte a:
#   - login com "usuario" ou "usuario@ebserh.gov.br"
#   - mensagens claras de senha
#   - scrypt para hashing
#   - domínios EBSERH automaticamente completados
#   - auditoria, sessões, bloqueio por falhas
# ======================================================================
...
# ======================================================================
def _email_normalizar(email: str) -> str:
...
def _email_ebserh_valido(email: str) -> bool:
    ...
    Aceita:
        jose.junior.95
        jose.junior.95@ebserh.gov.br
    Expande automaticamente.
    """
    e = _email_normalizar(email)

    # completar domínio automaticamente
    if "@" not in e:
        e = e + "@ebserh.gov.br"

    # validação mínima
    return e.endswith("@ebserh.gov.br") and " " not in e
...
def _registrar_auditoria(evento: str, usuario_id: Optional[int], email_norm: Optional[str], ip: Optional[str], user_agent: Optional[str]) -> None:
    conn = conectar(); cur = conn.cursor()
    cur.execute("""
        INSERT INTO auth_audit (usuario_id, email, evento, ip, user_agent)
        VALUES (?,?,?,?,?)
    """, (usuario_id, email_norm, evento, ip, user_agent))
    conn.commit(); conn.close()


def _login_bloqueado(email_norm: str, limite=5, janela_min=5) -> Tuple[bool, int]:
    conn = conectar(); cur = conn.cursor()
    cur.execute(f"""
        SELECT COUNT(*)
          FROM auth_audit
...
def _sessao_criar(usuario_id: int, duracao_horas: int = 8,
                  user_agent: Optional[str] = None,
                  ip: Optional[str] = None) -> str:
    token = secrets.token_urlsafe(32)
    conn = conectar(); cur = conn.cursor()
    cur.execute("""
        INSERT INTO sessoes (id, usuario_id, expira_em, user_agent, ip)
...
#  REGISTRO
# ======================================================================
def usuario_registrar(email: str, senha: str, nome: Optional[str] = None,
                      role: str = "user") -> int:

    email_norm = _email_normalizar(email)

    if "@" not in email_norm:
        email_norm = email_norm + "@ebserh.gov.br"
...
#  LOGIN
# ======================================================================
def usuario_login(email: str, senha: str,
                  user_agent: Optional[str] = None,
    ...
    if not _email_ebserh_valido(email_norm):
        raise ValueError("E-mail inválido. Use seu @ebserh.gov.br.")
    ...
    conn = conectar(); cur = conn.cursor()
    cur.execute("""
        SELECT id, email, senha_hash, nome, role, is_ativo
          FROM usuarios
         WHERE email = ?
    """, (email_norm,))
    row = cur.fetchone()
...
#  ALTERAR SENHA
# ======================================================================
def usuario_alterar_senha(usuario_id: int,
                          senha_atual: str,
                          nova_senha: str) -> None:
    ...
    cur.execute("""
        UPDATE usuarios
           SET senha_hash=?, atualizado_em=datetime('now','localtime')
         WHERE id=?
...
#  SEED ADMIN
# ======================================================================
def seed_admin_se_nao_existir(email_admin: str,
                              senha: str,
                              nome: Optional[str] = "Administrador") -> int:

    email_norm = _email_normalizar(email_admin)
    ...
    uid = usuario_registrar(email_norm, senha, nome=nome, role="admin")
    return uid
