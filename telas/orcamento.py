# telas/orcamento.py - APENAS REDUZIR PADDINGS
...
class TelaOrcamento(tk.Frame):
    def __init__(self, master):
        ...
        form.pack(fill="x", padx=12, pady=5)  # Reduzido de 8 para 5
        
        def campo(lbl, col, row, width=28):
            tk.Label(form, text=lbl).grid(column=col, row=row, sticky="w", padx=6, pady=2)  # Reduzido de 3 para 2
            e = ttk.Entry(form, width=width)
            e.grid(column=col + 1, row=row, sticky="w", padx=6, pady=2)  # Reduzido
            return e
        ...
        tk.Label(form, text="Observação:").grid(column=4, row=1, sticky="w", padx=6, pady=2)  # Reduzido
        self.e_obs = ttk.Entry(form, width=40)
        self.e_obs.grid(column=5, row=1, sticky="w", padx=6, pady=2)
        
        # Modelo rápido
        tk.Label(form, text="Modelo:").grid(column=0, row=2, sticky="w", padx=6, pady=2)  # Reduzido
        self.cb_modelo = ttk.Combobox(form, state="readonly", width=50)
        self.cb_modelo.grid(column=1, row=2, columnspan=3, sticky="w", padx=6, pady=2)  # Reduzido
        ttk.Button(form, text="Carregar", command=self._carregar_modelo_rapido).grid(column=4, row=2, padx=6)
        
        # Mensagem p/ email - altura reduzida
        tk.Label(form, text="Mensagem p/ e-mail:").grid(column=0, row=3, sticky="nw", padx=6, pady=2)  # Reduzido
        self.txt_msg = tk.Text(form, width=80, height=2)  # Reduzido de 4 para 2
        self.txt_msg.grid(column=1, row=3, columnspan=4, sticky="we", padx=6, pady=2)  # Reduzido
        ...
        self._lbl_autosave.grid(column=1, row=4, columnspan=5, sticky="w", padx=6, pady=(0, 2))  # Reduzido
        
        # ======================================================
        # BLOCO 2 — CAIXA LATERAL (COMPACTA)
        # ======================================================
        side = tk.Frame(form, bg="white")
        side.grid(column=5, row=0, rowspan=5, sticky="n", padx=(10, 6), pady=2)  # Reduzido
        
        ttk.Button(side, text="Add", command=self._adicionar).pack(fill="x", pady=(0, 6))  # Reduzido
        
        msg_box = ttk.LabelFrame(side, text="Mensagem: Modelo / Rascunho")
        msg_box.pack(fill="x")
        
        tk.Label(msg_box, text="Título:").pack(anchor="w", padx=6, pady=(4, 0))  # Reduzido
        self.e_titulo_msg = ttk.Entry(msg_box, width=25)  # Reduzido
        self.e_titulo_msg.pack(anchor="w", padx=6, pady=(0, 4))  # Reduzido
        ...
        ttk.Checkbutton(msg_box, text="Vincular ao fornecedor atual", variable=self.var_msg_forn)\
            .pack(anchor="w", padx=6, pady=(0, 4))  # Reduzido
        
        btns_msg = tk.Frame(msg_box, bg="white")
        btns_msg.pack(fill="x", padx=6, pady=(0, 6))  # Reduzido
        ...
        self.frm_anexos = tk.Frame(wrapper_anexos, bg="white", height=50)  # Reduzido de 60 para 50
        ...
        # ======================================================
        # BLOCO 3 — ABAS (COMPACTAS)
        # ======================================================
        lf_msg  = ttk.LabelFrame(form, text="Mensagens (Modelos e Rascunhos)")
        lf_msg.grid(column=1, row=5, columnspan=4, sticky="we", padx=6, pady=(4, 6))  # Reduzido
        
        busca_bar = tk.Frame(lf_msg)
        busca_bar.pack(fill="x", padx=6, pady=4)  # Reduzido
        ...
        msg_edit.pack(fill="x", padx=6, pady=(0, 4))  # Reduzido
        ...
        nb.pack(fill="both", expand=True, padx=6, pady=4)  # Reduzido
        ...
        self.tv_modelos = ttk.Treeview(aba_modelos, columns=cols_m, show="headings", height=4)  # Reduzido de 6 para 4
        ...
        bar_m.pack(fill="x", padx=4, pady=(0, 4))  # Reduzido
        ...
        bar_r.pack(fill="x", padx=4, pady=(0, 4))  # Reduzido
        ...
        # ======================================================
        # BLOCO 4 — ITENS EM RASCUNHO (COMPACTO)
        # ======================================================
        ...
        self.tv = ttk.Treeview(lf_rasc, columns=cols, show="headings", height=5)  # Reduzido de 6 para 5
        ...
        # ======================================================
        # BLOCO 5 — AÇÕES PRINCIPAIS (COMPACTO)
        # ======================================================
        rod = tk.Frame(self, bg="white")
        rod.pack(fill="x", padx=12, pady=5)  # Reduzido de 8 para 5
        ...
        # ======================================================
        # BLOCO 6 — HISTÓRICO (COMPACTO)
        # ======================================================
        lf_hist = ttk.LabelFrame(self, text="Orçamentos já salvos (no banco) — por fornecedor")
        lf_hist.pack(fill="both", expand=True, padx=12, pady=(2, 5))  # Reduzido
        
        filtros = tk.Frame(lf_hist)
        filtros.pack(fill="x", padx=6, pady=(4, 0))  # Reduzido
        ...
        self.tv_salvos = ttk.Treeview(lf_hist, columns=cols_s, show="headings", height=5)  # Reduzido de 8 para 5
        for c, h, w in zip(cols_s, heads_s, widths_s):
            self.tv_salvos.heading(c, text=h)
            self.tv_salvos.column(c, width=w, anchor="w")
        self.tv_salvos.pack(fill="both", expand=True, padx=6, pady=(4, 2))  # Reduzido
        
        barra_hist = tk.Frame(lf_hist, bg="white")
        barra_hist.pack(fill="x", padx=6, pady=(0, 4))  # Reduzido
        ...
        pag.pack(fill="x", padx=6, pady=(0, 4))  # Reduzido
